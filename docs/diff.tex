...
%DIF ADD paperin_review.tex        Tue Apr 18 20:35:01 2017
\usepackage{tikz} %DIF > 
%DIF -------
...
Even more unclear are }\DIFdelend \DIFaddbegin \DIFadd{The complexity of sizing these kinds of aircraft makes }\DIFaddend the trade offs between gas powered and \DIFdelbegin \DIFdel{solar-electric powered architectures }\DIFdelend \DIFaddbegin \DIFadd{solar-powered architectures even more unclear}\DIFaddend . 

This paper presents a physics-based optimization model that uses geometric programming as a systematic and rapid approach to \DIFdelbegin \DIFdel{explore this design space and highlight }\DIFdelend \DIFaddbegin \DIFadd{evaluate }\DIFaddend trade offs between the two \DIFdelbegin \DIFdel{proposed architectures}\DIFdelend \DIFaddbegin \DIFadd{archictures for a given mission}\DIFaddend .  Geometric programming, a form of convex optimization, is chosen as a means of evaluating the design space because of its rapid solve time and guaranteed convergence to a global optimum.\cite{gp}
...
The optimization is solved multiple times \DIFdelbegin \DIFdel{for different design parameters such as endurance, latitude, and percentile wind speed }\DIFdelend to observe how aircraft size and weight are affected by different \DIFaddbegin \DIFadd{design parameters and }\DIFaddend requirements. 
...
\subsection{\DIFadd{Payload}}

\DIFadd{A 10 lbs payload is a baseline requirement for both the gas and solar-electric powered aircraft. This requirement can be changed to observe how each architeure is able to handle larger or smaller payloads.
}

\DIFadd{Inherent in a payload requirement is the ability to station keep. }\DIFaddend To station keep an aircraft must fly at least as fast as the wind speed,
...
For a solar-electric powered aircraft, however, if it can fly during the shortest day of the year, December 21st \DIFaddbegin \DIFadd{(June 21st for Southern Hemisphere)}\DIFaddend , then it can thoretically fly at any time of the year\DIFaddbegin \DIFadd{, more than satisfying the 6 day endurance requirement}\DIFaddend . 
Therefore, \DIFaddbegin \DIFadd{to meet }\DIFaddend the endurance requirement \DIFdelbegin \DIFdel{for }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend solar-electric powered aircraft \DIFdelbegin \DIFdel{is assumed to be a requirement }\DIFdelend \DIFaddbegin \DIFadd{must be able }\DIFaddend to fly during the winter solstice.  
...
Because \DIFdelbegin \DIFdel{the endurance requirement for solar-electric powered aircraft is set by the winter solstice and because }\DIFdelend \DIFaddbegin \DIFadd{becuase }\DIFaddend wind speeds are \DIFdelbegin \DIFdel{higher during winter months, only wind speed data from December }\DIFdelend \DIFaddbegin \DIFadd{greatest during the winter solstice for both the Northern and Southern Hemispheres only winds speeds from December and June }\DIFaddend will be used for the aircraft sizing and performance analysis presented in this paper. 
...
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \begin{subfigmatrix}{2}%DIF >  number of columns
 ...
 \end{subfigmatrix}
 \DIFaddendFL \caption{\DIFdelbeginFL \textbf{\DIFdelFL{Winds at 30$^{\circ}$ latitude.  Bands represent 80th, 90th and 95th percentiles. Winds peak in winter.}}%DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{Winds peak at summer and winter solstice in Southern and Northern Hemispheres respectively.\mbox{%DIFAUXCMD
\cite{wind}}%DIFAUXCMD
}}\DIFaddendFL }
 \label{f:windvsmonth}
...
\DIFaddend \subsection{Altitude}

\DIFdelbegin \DIFdel{Gas powered aircraft generally fly between 5,000 and 25}\DIFdelend \DIFaddbegin \DIFadd{The altitude requirement is generally set by a minimum payload required to meet a certain coverage
}
...
\DIFadd{Long-endurance aircraft tend to fly at low velocities, which based on Figure~\ref{f:altvswindhmin} means there are two possible operating altitude regiems: at or around the minimum payload requirement 4}\DIFaddend ,\DIFaddbegin \DIFadd{600 - 7,600 m or at high altitudes between 16,700-20,}\DIFaddend 000 \DIFdelbegin \DIFdel{ft due to payload requirements. 
Because of the limitations of }\DIFdelend \DIFaddbegin \DIFadd{m. 
Gas powered aircraft generally fly in the lower altitude regime of 4,600-7,600 m because }\DIFaddend naturally aspirated engines \DIFdelbegin \DIFdel{and linearly increasing wind speeds from 0 to 35}\DIFdelend \DIFaddbegin \DIFadd{lose power with increased altitude and are unable to reach altitudes higher than about 13}\DIFaddend ,\DIFdelbegin \DIFdel{000 ft shown in Figure~\ref{f:altvswind}, gas powered aircraft will generally be more efficient at lower altitudes and generally fly at a given altitude specified by the mission.  
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{500 m depending on the engine.  
}\DIFaddend Solar-electric powered aircraft\DIFdelbegin \DIFdel{tend }\DIFdelend \DIFaddbegin \DIFadd{, which do not have naturally aspirated engines, will }\DIFaddend to fly around \DIFdelbegin \DIFdel{60}\DIFdelend \DIFaddbegin \DIFadd{18}\DIFaddend ,000 \DIFdelbegin \DIFdel{ft to meet payload requirements, }\DIFdelend \DIFaddbegin \DIFadd{m to }\DIFaddend avoid cloud coverage\DIFdelbegin \DIFdel{and fly in lower wind speeds which have a minimum around 60,000 shown in Figure~\ref{f:altvswind}.
...
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.6\textwidth,natwidth=538,natheight=405]{altvswindhmin.pdf}
...
\DIFaddbegin \DIFadd{It is assumed that long-endurance aircraft will have a requirement to fly within a band of latitudes (i.e. $\pm$45 degrees latitude).  
...
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.6\textwidth,natwidth=576,natheight=432]{latvswindN.pdf}
...
\DIFadd{To evaluate the gas and solar-electric powered design space, the requirements, architecture type, physical models, and assummed values }\DIFaddend are expressed as constraints on the GP.\cite{hoburgthesis} 
...
\DIFaddbegin \DIFadd{Various structural, aerodynamic, and weight models necesistate assumptions about the vehicle configuration.  
...
\begin{figure}[H]
    ...
    \caption{\textbf{\DIFaddFL{Blue and red are specific to the solar-electric and gas archictures respectively.  Black are shared characteristics.}}}
...
\DIFadd{These assumptions can be changed to evaulate different configuration.  Changes in configuration assumptions may require modification of models or equations. 
}
...
\subsection{\DIFdelbegin \DIFdel{Wind Speeds}\DIFdelend \DIFaddbegin \DIFadd{Solar-Electric Aircraft Physical Models}\DIFaddend }

\DIFaddbegin \subsubsection{\DIFadd{Wind Speeds}}

\DIFaddend There exists an optimum flight altitude for solar-electric powered aircraft.  
The local minimum in wind speed around \DIFdelbegin \DIFdel{63}\DIFdelend \DIFaddbegin \DIFadd{19}\DIFaddend ,000 \DIFdelbegin \DIFdel{ft }\DIFdelend \DIFaddbegin \DIFadd{m }\DIFaddend and the variation of air density with altitude suggest that the altitude should be an output of the optimization. 
...
\DIFdelbegin \DIFdel{Additionally, because long-endurance aircraft are likely to fly over land and the majority of land mass lies in the Northern Hemisphere , only wind speed data from the Northern Hemisphere is considered}\DIFdelend \DIFaddbegin \DIFadd{The wind speed data used to generate these equations includes December wind speeds from the Northern Hemisphere and June wind speeds from the Southern Hemisphere for the year 2005-2015}\DIFaddend .
In total, 41 wind speed constraints were generated \DIFdelbegin \DIFdel{from }\DIFdelend \DIFaddbegin \DIFadd{to represent $\pm$}\DIFaddend 20-60 degrees\DIFdelbegin \DIFdel{North latitude}\DIFdelend . (See Appendix A) 
The constraint on December wind speeds and air density at the \DIFdelbegin \DIFdel{35 }\DIFdelend \DIFaddbegin \DIFadd{30 }\DIFaddend degree latitude is 

\begin{align}
    \left(\frac{V_{\text{wind}}}{V_{\text{ref}}}\right)\DIFdelbegin \DIFdel{^{7.213} }\DIFdelend \DIFaddbegin \DIFadd{^{7.066} }\DIFaddend &\geq \DIFdelbegin \DIFdel{2918}\DIFdelend \DIFaddbegin \DIFadd{18413.2}\DIFaddend \rho\DIFdelbegin \DIFdel{^{7.587}}\DIFdelend \DIFaddbegin \DIFadd{^{9.3}}\DIFaddend p_{\text{wind}}\DIFdelbegin \DIFdel{^{9.37} }\DIFdelend \DIFaddbegin \DIFadd{^{5.51} }\DIFaddend + \DIFdelbegin %DIFDELCMD < \num{1.58d-10}%%%
...
\begin{figure}[H]
	\begin{center}
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.6\textwidth,natwidth=511,natheight=405]{windfitl35.pdf}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.6\textwidth,natwidth=511,natheight=405]{windfitl30.pdf}
    \DIFaddendFL \caption{\DIFdelbeginFL \textbf{\DIFdelFL{Posynomial approximation to wind speed data at 35$^{\circ}$ latitude. (Log-space RMS error = 0.043.)}}%DIFAUXCMD
\DIFdelendFL \DIFaddbeginFL \textbf{\DIFaddFL{Posynomial approximation to wind speed data\mbox{%DIFAUXCMD
\cite{wind} }%DIFAUXCMD
(circles) at 35$^{\circ}$ latitude. (Log-space RMS error = 0.043.)}}\DIFaddendFL }
	\label{f:windfitl35}
	\end{center}
\end{figure}
...
Additionally, wind speeds increase nearly linearly up to \DIFdelbegin \DIFdel{30,000 ft}\DIFdelend \DIFaddbegin \DIFadd{9,100 m}\DIFaddend .  
...
For this sizing study, the altitude requirement for the gas powered aircraft is \DIFdelbegin \DIFdel{$h=15,000$ ft}\DIFdelend \DIFaddbegin \DIFadd{$h=4,500$ m}\DIFaddend , corresponding to a 100 km diameter footprint with a 5 degree lookup angle. 

\DIFdelbegin \subsection{\DIFdel{Solar-Electric Power}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin \subsubsection{\DIFadd{Solar-Electric Power}}
\DIFaddend 
...
\DIFdelbegin \DIFdel{Figure~\ref{f:energyapprox} shows sample monomial approximations at 30 degrees latitude. 
}\DIFdelend %DIF >  Figure~\ref{f:energyapprox} shows sample monomial approximations at 30 degrees latitude. 
%DIF >  
%DIF >  \begin{figure}[H]
%DIF >   \begin{subfigmatrix}{2}% number of columns
...
\subsection{\DIFdelbegin \DIFdel{Breguet Endurance}\DIFdelend \DIFaddbegin \DIFadd{Gas Powered Aircraft Physical Models}\DIFaddend }

\DIFaddbegin \subsubsection{\DIFadd{Breguet Endurance}}
...
\DIFaddbegin \subsubsection{\DIFadd{Elliptcal Fuselage}}

\DIFadd{For the gas powered aircraft it is assumed that the fuel is carried in an elliptically shaped fuselage.  The fuselage will increase the overall weight and drag of the aircraft.  The solar-electric powered aircraft is assumed to carry the batteries in the wings will therefore have a small fuselage whose effects will be ignored.  
}

\DIFadd{The driving constraint for the size of the fuselage is to ensure that all of the fuel required for the mission can fit inside the fuselage, 
}
...
\DIFadd{where the fuel is assumed to have a density $\rho_\text{fuel} = 6.01$ }[\DIFadd{lbf/gallon}]\DIFadd{.  The dimensions of the fuselage are constrained by
}
...
\DIFadd{where $l_{\text{fuse}}$ is the length of the fuselage and $R_{\text{fuse}}$ is the radius. Using the length and radius, the surface area can be calculated using Thomsen's approximation,\mbox{%DIFAUXCMD
\cite{ellipsoidSA}
}%DIFAUXCMD
}

\begin{equation}
    \DIFadd{\label{e:fusesa}
    3 \left( \frac{S_{\text{fuse}}}{\pi} \right)^{1.6075} \geq 2(2l_{\text{fuse}}R_{\text{fuse}})^{1.6075} + (4R_{\text{fuse}}^2)^{1.6075}.
}\end{equation}

\DIFadd{The weight of the fuselage is constrained by
}

\begin{equation}
    \DIFadd{\label{e:fuseweight}
    W_{\text{fuse}} \geq S_{\text{fuse}} \rho_{A_{\text{cfrp}}} g
}\end{equation} 

\DIFadd{where $\rho_{A_{\text{cfrp}}} = 0.0975$ }[\DIFadd{g/cm$^2$}]\DIFadd{, or the area density of two plys of carbon fiber.\mbox{%DIFAUXCMD
\cite{cfply}  }%DIFAUXCMD
The surface area is also used to calculate the drag assuming a skin friction based drag model,
}

\begin{align}
    \DIFadd{\label{e:fusedrag}
    D_{\text{fuse}} }&\DIFadd{\geq C_f k_{\text{fuse}} \frac{1}{2} \rho V^2 S_{\text{fuse}} }\\
    \DIFadd{C_f }&\DIFadd{\geq \frac{0.455}{Re^{0.3}}
}\end{align}

\DIFadd{where $k_{\text{fuse}}$ is the form factor approximated by\mbox{%DIFAUXCMD
\cite{raymer}
}%DIFAUXCMD
}

\begin{equation}
    \DIFadd{\label{e:fuseform}
    k_{\text{fuse}} \geq 1 + \frac{60}{(l_{\text{fuse}}/2R_{\text{fuse}})^3} + \frac{(l_{\text{fuse}}/2R_{\text{fuse}})}{400}.
}\end{equation}

\subsubsection{\DIFadd{Engine Weight}}
...
\DIFadd{where $m_{\text{motor}}$ is the motor mass, $P_{\text{max}} \geq P_{\text{oper}}$ is the maximum operating power, and the power to mass ratio is $B_{PM} = 4140.8$ }[\DIFadd{W/kg}]\DIFadd{. }\\

\DIFadd{The engine weight of the gas powered aircraft is governed by a simple power law derived from existing two-stroke and four-stroke engines\mbox{%DIFAUXCMD
\cite{gasengine}
}%DIFAUXCMD
}

\begin{equation}
    \DIFadd{\label{e:powerlaw}
    \frac{W_{\text{engine}}}{W_{\text{engine-ref}}} = 1.27847 \left(\frac{P_{\text{SL-max}}}{P_{\text{ref}}} \right)^{0.772392}
}\end{equation}

\DIFadd{where $W_{\text{engine-ref}} = 10$ }[\DIFadd{lbs}] \DIFadd{and $P_{\text{ref}} = 10$ }[\DIFadd{hp}]\DIFadd{.  Equation~}\eqref{e:powerlaw} \DIFadd{was found using techniques described in Hoburg et. al\mbox{%DIFAUXCMD
\cite{fitting}}%DIFAUXCMD
. Equation~}\eqref{e:powerlaw} \DIFadd{is compared to the data in Figure~\ref{f:powervsweightfit}.
}

\begin{figure}[H]
	\begin{center}
	\includegraphics[width=0.6\textwidth,natwidth=524,natheight=405]{powervsweightfit.pdf}
    \caption{\textbf{\DIFaddFL{Power law fit to University of North Dakota engine weight data\mbox{%DIFAUXCMD
\cite{gasengine}}%DIFAUXCMD
. (Log-space RMS error = 0.34)}}}
	\label{f:powervsweightfit}
	\end{center}
\end{figure}

\subsubsection{\DIFadd{Gas Powered Engine Performance}}

\DIFadd{Two characteristics of gas engines affect the performance of long-endurance aircraft.  
...
RCV Engines Ltd provided manufacturing data that was used to generate representative performance curves.\mbox{%DIFAUXCMD
\cite{rcvengines}
}%DIFAUXCMD
...
\DIFadd{To capture throttling effects as aircraft weight decreases, a GP-compatible curve was generated from the DF70 data to relate $\text{BSFC}$ and shaft power, 
}

\begin{equation}
    \DIFadd{\label{e:rpmtobsfc}
...
\DIFadd{where $\text{BSFC}_{100\%} = 0.3162$ }[\DIFadd{kg/kW/hr}]\DIFadd{.\mbox{%DIFAUXCMD
\cite{rcvengines}
}%DIFAUXCMD
Using this approximation the required shaft power determines the $\text{BSFC}$.
A comparison of the GP-compatible approximation to the manufacturing data from RCV engines is presented in Figure~\ref{f:powervsweightfit}. A comparison of the DF70 data provided by RCV Engine Ltd in Figure~\ref{f:powervsweightfit} to the $\text{BSFC}$ to power curves in Goering\mbox{%DIFAUXCMD
\cite{bsfcperf} }%DIFAUXCMD
verify that this performance curve is representative of engines of similar sizes. 
}

\begin{figure}[H]
	\begin{center}
	\includegraphics[width=0.6\textwidth,natwidth=530,natheight=414]{powertobsfcfit.pdf}
    \caption{\textbf{\DIFaddFL{Representative engine performance fit based on RCV Engine Ltd data.  (Log-space RMS error = 0.007) }}}
	\label{f:powervsweightfit}
	\end{center}
\end{figure}

\DIFadd{The lapse rate $L_{\text{eng}}(h) \leq 1$, is assumed to affect the maximum power output,
}

\begin{equation}
    \DIFadd{\label{e:lapse}
    L_{\text{eng}}(h) \equiv \frac{P_{\text{max}}}{P_{\text{SL-max}}}.
}\end{equation}

\DIFadd{The lapse is calculated from the required flight altitude $h=4,500$ }[\DIFadd{m}]\DIFadd{, prior to the optimization solve using an approximate engine loss rate for normally aspirated engines of 3.5\% hp per 300 m,\mbox{%DIFAUXCMD
\cite{enginelapse}
}%DIFAUXCMD
}

\begin{equation}
    \DIFadd{\label{e:lapsefit}
    L_{\text{eng}}(h) = 1 - \frac{0.035}{300 \text{ [m]}} h.
}\end{equation}

\subsubsection{\DIFadd{Climb Constraints}}

\DIFadd{Because the gas engine is naturally aspirated, as the aircraft climbs there will be less available power.  
...
\begin{equation}
    \DIFadd{\label{e:climbrate}
    \dot{h} \geq 30 \text{ [m/min]}.
}\end{equation}

\DIFadd{The climb rate affects the required thrust, and therefore the required power during climb, 
}

\begin{equation}
    \DIFadd{\label{e:climb}
    T \geq \frac{1}{2} C_D \rho V^2 S + W \frac{\dot{h}}{V}
}\end{equation}

\DIFadd{where $W$ is the weight of the aircraft during climb.  
}

\subsection{\DIFadd{Shared Physical Models}}

\DIFadd{The models described in this section were applied to both the gas powered and solar-electric powered aircraft models. 
}

\DIFaddend \subsection{Steady Level Flight}
...
Using a standard Bernoulli-Euler discretized beam model with $n=5$ nodes, the shear forces\DIFdelbegin \DIFdel{and momentscan be computed from }\DIFdelend \DIFaddbegin \DIFadd{, moments, angles, and deflections can be expressed in a GP-compatible form using }\DIFaddend the distributed loads $q(y)$, \DIFdelbegin \DIFdel{with boundary conditions of }\DIFdelend \DIFaddbegin \DIFadd{as an input. Boundary conditions are }\DIFaddend zero shear forces and moments at the wing tips \DIFdelbegin \DIFdel{.\mbox{%DIFAUXCMD
\cite{bending}
}%DIFAUXCMD
}%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdel{Similarly, the angle deflection and deflection can be calculated with boundary conditions of }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend zero angle and deflection \DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{at }\DIFaddend the wing root.\cite{bending}
...
The ultimate compressive strength for carbon fiber is, \DIFdelbegin \DIFdel{$\sigma_{\text{cfrp}} = 570$ }\DIFdelend \DIFaddbegin \DIFadd{$\sigma_{\text{cfrp}} = 1700$ }\DIFaddend [MPa].\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{carbonfiber}
}%DIFAUXCMD
...
\begin{align}
    ...
    \DIFdelbegin \DIFdel{D_{\text{v}} }%DIFDELCMD < &%%%
\DIFdel{\geq \frac{1}{2} c_{d_{\text{v}}} \rho V^2 S_{\text{v}} }%DIFDELCMD < \\
%DIFDELCMD <     %%%
...
\DIFdelbegin \DIFdel{Re_{\text{v}} }%DIFDELCMD < &%%%
\DIFdel{= \frac{V\rho S_{\text{v}}/b_{\text{v}}}{\mu} }%DIFDELCMD < \\
%DIFDELCMD <     %%%
\DIFdel{Re_{\text{h}} }%DIFDELCMD < &%%%
...
\DIFdelbegin \DIFdel{A comparison of the XFOIL data and Equation~}%DIFDELCMD < \eqref{e:taildrag} %%%
\DIFdel{is shown in Figure~\ref{f:taildragpolar}.
}\DIFdelend %DIF >  A comparison of the XFOIL data and Equation~\eqref{e:taildrag} is shown in Figure~\ref{f:taildragpolar}.

\DIFdelbegin %DIFDELCMD < \begin{figure}[H]
%DIFDELCMD < 	\begin{center}
%DIFDELCMD < 	\includegraphics[width=0.6\textwidth,natwidth=538,natheight=405]{taildragpolar.pdf}
%DIFDELCMD <     %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\textbf{\DIFdelFL{Posynomial fit (solid lines) to XFOIL data (circles). (Log-space RMS error = 0.0173)}}%DIFAUXCMD
}
	%DIFAUXCMD
%DIFDELCMD < \label{f:taildragpolar}
%DIFDELCMD < 	\end{center}
%DIFDELCMD < \end{figure}
%DIFDELCMD < %%%
...
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend %DIF >  \subsection{Weight Breakdown}
%DIF >  
...
\DIFdelbegin \DIFdel{For the gas powered aircraft it is assumed that the fuel is carried in an elliptically shaped fuselage.  The fuselage will increase the overall weight and drag of the aircraft.  The }\DIFdelend \DIFaddbegin \section{\DIFadd{Results}}

\DIFadd{The gas and }\DIFaddend solar-electric powered aircraft \DIFdelbegin \DIFdel{is assumed to carry the batteries in the wings will therefore have a small fuselage whose effects will be ignored.  
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The driving constraint for the size of the fuselage is to ensure that all of the fuel required for the mission can fit inside the fuselage, 
}%DIFDELCMD < 

%DIFDELCMD < %%%
\begin{displaymath}
    \DIFdel{\label{e:fusevol}
    \mathcal{V}_{\text{fuse}} \geq \frac{W_\text{fuel}}{\rho_\text{fuel}}
}\end{displaymath}
%DIFAUXCMD
%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdel{where the fuel is assumed to have a density $\rho_\text{fuel} = 6.01$ }%DIFDELCMD < [%%%
\DIFdel{lbf/gallon}%DIFDELCMD < ]%%%
\DIFdel{.  The dimensions of the fuselage are constrained by }%DIFDELCMD < 

%DIFDELCMD < %%%
\begin{displaymath}
    \DIFdel{\label{e:fusevol2}
    \mathcal{V}_{\text{fuse}} \leq \frac{4}{3}\pi \frac{l_{\text{fuse}}}{2}R_{\text{fuse}}^2
}\end{displaymath}
%DIFAUXCMD
%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdel{where $l_{\text{fuse}}$ is the length of the fuselage and $R_{\text{fuse}}$ is the radius. Using the length and radius, the surface area can be calculated using Thomsen's approximation,\mbox{%DIFAUXCMD
\cite{ellipsoidSA}
}%DIFAUXCMD
}%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdel{The weight of the fuselage is constrained by
}%DIFDELCMD < 

%DIFDELCMD < %%%
\begin{displaymath}
    \DIFdel{\label{e:fuseweight}
    W_{\text{fuse}} \geq S_{\text{fuse}} \rho_{A_{\text{cfrp}}} g
}\end{displaymath} 
%DIFAUXCMD
%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdel{where $\rho_{A_{\text{cfrp}}} = 0.0975$ }%DIFDELCMD < [%%%
\DIFdel{g/cm$^2$}%DIFDELCMD < ]%%%
\DIFdel{, or the area density of two plys of carbon fiber. 
\mbox{%DIFAUXCMD
\cite{carbonfiber}  }%DIFAUXCMD
The surface area is also used to calculate the drag assuming a skin friction based drag model,
}%DIFDELCMD < 
...
%DIFDELCMD < %%%
\begin{eqnarray*}
    \DIFdel{\label{e:fusedrag}
    D_{\text{fuse}} }&\DIFdel{\geq C_f k_{\text{fuse}} \frac{1}{2} \rho V^2 S_{\text{fuse}} }\\
    \DIFdel{C_f }&\DIFdel{\geq \frac{0.455}{Re^{0.3}}
}\end{eqnarray*}
%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{where $k_{\text{fuse}}$ is the form factor approximated by\mbox{%DIFAUXCMD
\cite{raymer}
}%DIFAUXCMD
}%DIFDELCMD < 
...
%DIFDELCMD < %%%
\subsection{\DIFdel{Engine Weight}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{optimization models were solved by minimizing take of weight for the requirements listed in Table~\ref{t:mreqs}. 
Key design parameters for both aircraft optimization model are listed in Table~\ref{t:gassolarparams}.
}\DIFaddend The \DIFdelbegin \DIFdel{solar-electric powered aircraft has a motor whose weight is based on the approximation\mbox{%DIFAUXCMD
\cite{electricengine}
}%DIFAUXCMD
}%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdel{The engine weight of the }\DIFdelend gas powered aircraft \DIFdelbegin \DIFdel{is governed by a simple power law derived from existing two-stroke and four-stroke engines\mbox{%DIFAUXCMD
\cite{gasengine}
}%DIFAUXCMD
}%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdel{where $W_{\text{engine-ref}} = 10$ }%DIFDELCMD < [%%%
\DIFdel{lbs}%DIFDELCMD < ] %%%
\DIFdel{and $P_{\text{ref}} = 10$ }%DIFDELCMD < [%%%
\DIFdel{hp}%DIFDELCMD < ]%%%
\DIFdel{.  Equation~}%DIFDELCMD < \eqref{e:powerlaw} %%%
\DIFdel{was found using techniques described in Hoburg et. al\mbox{%DIFAUXCMD
\cite{fitting}}%DIFAUXCMD
. Equation~}%DIFDELCMD < \eqref{e:powerlaw} %%%
\DIFdel{is compared to the data in Figure~\ref{f:powervsweightfit}.
}%DIFDELCMD < 

%DIFDELCMD < \begin{figure}[H]
%DIFDELCMD < 	\begin{center}
%DIFDELCMD < 	\includegraphics[width=0.6\textwidth,natwidth=524,natheight=405]{powervsweightfit.pdf}
%DIFDELCMD <     %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\textbf{\DIFdelFL{Power law fit to University of North Dakota engine weight data\mbox{%DIFAUXCMD
\cite{gasengine}}%DIFAUXCMD
. (Log-space RMS error = 0.34)}}%DIFAUXCMD
}
	%DIFAUXCMD
%DIFDELCMD < \label{f:powervsweightfit}
%DIFDELCMD < 	\end{center}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\subsection{\DIFdel{Gas Powered Engine Performance}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Two characteristics of gas engines affect the performance of long-endurance aircraft.  
The first is brake specific fuel consumption, $\text{BSFC}$; a lower $\text{BSFC}$ will result in increased endurance.  
The second is the lapse rate.  
Assuming a propeller driven aircraft and a naturally aspirated engine, as the aircraft reaches higher altitudes, the engine will have decreased available power. 
To account for these two affects a two-stroke, double cylinder engine, the DF70 from RCV Engines Ltd, England, was selected as a representative engine.  
...
%DIFDELCMD < %%%
\subsubsection{\DIFdel{Brake Specific Fuel Consumption Constraint}}
%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{To capture throttling effects as aircraft weight decreases, a GP-compatible curve was generated from the DF70 data to relate $\text{BSFC}$ and shaft power, 
}%DIFDELCMD < 
...
Using this approximation the required shaft power determines the $\text{BSFC}$.
A comparison of the GP-compatible approximation to the manufacturing data from RCV engines is presented in Figure~\ref{f:powervsweightfit}.  
A comparison of the DF70 data provided by RCV Engine Ltd in Figure~\ref{f:powervsweightfit} to the $\text{BSFC}$ to power curves in Goering\mbox{%DIFAUXCMD
\cite{bsfcperf} }%DIFAUXCMD
verify that this performance curve is representative of engines of similar sizes. 
}%DIFDELCMD < 
...
%DIFDELCMD < \begin{figure}[H]
%DIFDELCMD < 	\begin{center}
%DIFDELCMD < 	\includegraphics[width=0.6\textwidth,natwidth=530,natheight=414]{powertobsfcfit.pdf}
%DIFDELCMD <     %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\textbf{\DIFdelFL{Representative engine performance fit based on RCV Engine Ltd data.  (Log-space RMS error = 0.007) }}%DIFAUXCMD
}
	%DIFAUXCMD
%DIFDELCMD < \label{f:powervsweightfit}
%DIFDELCMD < 	\end{center}
...
%DIFDELCMD < %%%
\subsubsection{\DIFdel{Engine Lapse Rate Calculation}}
%DIFAUXCMD
\addtocounter{subsubsection}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The lapse rate $L_{\text{eng}}(h) \leq 1$, is assumed to affect the maximum power output,
}%DIFDELCMD < 

%DIFDELCMD < %%%
\begin{displaymath}
    \DIFdel{\label{e:lapse}
    L_{\text{eng}}(h) \equiv \frac{P_{\text{max}}}{P_{\text{SL-max}}}.
}\end{displaymath}
%DIFAUXCMD
%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdel{The lapse is calculated from the required flight altitude $h=15,000$ }%DIFDELCMD < [%%%
\DIFdel{ft}%DIFDELCMD < ]%%%
\DIFdel{, prior to the optimization solve using an approximate engine loss rate for normally aspirated engines of 3.5\% hp per 1000 ft,\mbox{%DIFAUXCMD
\cite{enginelapse}
}%DIFAUXCMD
}%DIFDELCMD < 
...
%DIFDELCMD < %%%
\subsection{\DIFdel{Climb Constraints}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Because the gas engine is naturally aspirated, as the aircraft climbs there will be less available power.  
The climb constraint ultimately sizes the engine because at the top of climb, when the least amount of power is available, the engine must provide the necessary power to meet a minimum climb rate, 
}%DIFDELCMD < 

%DIFDELCMD < %%%
\begin{displaymath}
    \DIFdel{\label{e:climbrate}
...
%DIFDELCMD < %%%
\DIFdel{The climb rate affects the required thrust, and therefore the required power during climb, 
}%DIFDELCMD < 

%DIFDELCMD < %%%
\begin{displaymath}
    \DIFdel{\label{e:climb}
    T \geq \frac{1}{2} C_D \rho V^2 S + W \frac{\dot{h}}{V}
}\end{displaymath}
%DIFAUXCMD
%DIFDELCMD < 
...
%DIFDELCMD < %%%
\DIFdel{where $W$ is the weight of the aircraft during climb.  
}%DIFDELCMD < 

%DIFDELCMD < %%%
\subsection{\DIFdel{Weight Breakdown}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The weight of each component of both the gas and solar platforms is summed to constrain the }\DIFdelend \DIFaddbegin \DIFadd{optimization model had 552 unknowns and was solved in 0.144 seconds for a max take off weight of 84.0 lbs.  
The solar-electric powered optimization model had 375 unknowns and was solved in 0.164 seconds for a }\DIFaddend max take off weight \DIFdelbegin \DIFdel{.  
The structural weight comprises the weight of the wing, fuselage, horizontal and vertical stabilizers, and engine. 
The weight breakdown for }\DIFdelend \DIFaddbegin \DIFadd{of 320.5 lbs.
According to the assumptions described in the previous section, }\DIFaddend the gas powered aircraft is \DIFdelbegin \DIFdel{a summation of the structural weight, the total fuel weight, and the payload weight}\DIFdelend \DIFaddbegin \DIFadd{better suited to meet this specific set of requirements. 
...
\DIFadd{$\tau$                       }& \DIFadd{0.5  }& \DIFadd{$\tau$                       }& \DIFadd{0.115 }\\
...
\DIFadd{$\dot{h}$ }\DIFaddend [\DIFdelbegin \DIFdel{lbs}\DIFdelend \DIFaddbegin \DIFadd{ft/min}\DIFaddend ]           \DIFdelbegin \DIFdel{.
}\DIFdelend \DIFaddbegin & \DIFadd{100 }& \DIFadd{$\eta_{\text{motor}}$           }& \DIFadd{0.95 }\\ 
                             &     & \DIFadd{$\eta_{\text{charge}}$          }& \DIFadd{0.98 }\\
...
\DIFdelbegin \DIFdel{To compare the gas-powered }\DIFdelend \DIFaddbegin \DIFadd{Important trade offs between the gas powered }\DIFaddend and solar-electric \DIFdelbegin \DIFdel{architectures the }\DIFdelend \DIFaddbegin \DIFadd{powered archictures are highlighted and quantified by changing the latitude and endurance endurance requirements.   
Both }\DIFaddend optimization models were solved by minimizing the max take off weight across different bands of latitudes. 
...
\DIFdelbegin \DIFdel{Each point on the graph is a unique solution to the optimization model for the stated assumptions and constants}\DIFdelend \DIFaddbegin \DIFadd{The gas powered model was solved x times and the solar-electric powered model was solved x times to produce Figure~\ref{f:latvsmtowtrade}}\DIFaddend .
...
One way to interpret Figure~\ref{f:latvsmtowtrade} is that a solar-powered aircraft weighing 190 lbs is able to operate between $\pm$30 degrees \DIFdelbegin \DIFdel{North }\DIFdelend latitude in 90th percentile wind speeds.  
This analysis shows that gas powered architectures are able to operate in more locations than solar-electric powered aircraft.  
The gas powered aircraft curves level off around 38 degrees latitude because wind speeds are highest at that latitude at \DIFdelbegin \DIFdel{15,000 ft}\DIFdelend \DIFaddbegin \DIFadd{4,500 m}\DIFaddend . 
...
Solving the gas powered optimization model for different endurance requirements shows where the gas powered archicture becomes less feasible}\DIFaddend . 
...
\subsection{\DIFadd{Changing Input Values}}
...
Figure~\ref{f:battsolarcontour} shows contours of latitude for a given solar cell efficiency and battery energy density.  Put another way, this plot shows how good the }\DIFaddend solar \DIFdelbegin \DIFdel{cell area the airplane is slightly bigger but requires fewer solar cells . (Figure~\ref{f:solarobjcomp})
...
\DIFdelendFL \DIFaddbeginFL \begin{center}
    ...
    \caption{\textbf{\DIFaddFL{Contours of latitude. Reaching higher latitudes required better solar cells and batteries.}}}
...
%DIFDELCMD <  %%%
\DIFdelendFL \DIFaddbeginFL \subfigure[35th Latitude, 80\% Wind Speed]{\includegraphics[natwidth=542,natheight=414]{bcontourl35a80.pdf}}
...
\subsection{\DIFadd{Altitude and relevant atmosphereic quantities}}

\DIFadd{The altitude also determines atmospheric properties such as air density and viscosity.  For the gas powered aircraft air density is calculated prior to solving the optimization using the altitude requirement and atmospheric equations specific to the troposphere,\mbox{%DIFAUXCMD
\cite{isaatm} 
}%DIFAUXCMD
}
...
\DIFadd{Air viscosity is also computed prior to solving and is assumed to vary according to Sutherland's law,\mbox{%DIFAUXCMD
\cite{fluiddyhandbook}
}%DIFAUXCMD
}

\begin{equation}
    \DIFadd{\label{e:sutherland}
    \mu = \mu_0 \frac{T_0 + C}{T+C} \left( \frac{T}{T_0} \right)^{\frac{3}{2}}
}\end{equation}

\DIFadd{where $\mu_0 = 1.827\text{e}-5$ }[\DIFadd{Ns/m$^2$}]\DIFadd{, $T_0 = 291.15$ }[\DIFadd{K}]\DIFadd{, and $C = 120$ }[\DIFadd{K}]\DIFadd{. }\\

\DIFadd{For the solar aircraft altitude is calculated based off of the optimized air density using the relations,\mbox{%DIFAUXCMD
\cite{isaatm} 
}%DIFAUXCMD
}

\begin{align}
    \DIFadd{\label{e:tropopress}
    p }&\DIFadd{= \rho R_{\text{spec}}T_{11} }\\
    \DIFadd{\label{e:tropoalt}
    h }&\DIFadd{= 11,000 \text{ [m] } + \frac{R_{\text{spec}}T_{11}}{g}\frac{p}{p_{11}}.
}\end{align}

\DIFadd{According to Sutherland's law\mbox{%DIFAUXCMD
\cite{fluiddyhandbook}}%DIFAUXCMD
, viscosity varies with temperature.  Because temperature is assumed constant 11,000-20,000 meters\mbox{%DIFAUXCMD
\cite{isaatm}}%DIFAUXCMD
, viscosity is also assumed to be constant, $\mu=\num{1.42d-5}$. }\\
\DIFaddend 

...
Observations in wind speed data\cite{wind} indicate that it is unlikely for long-endurance UAVs to be able to station keep in 100 percentile wind speeds.  
Therefore, the station keeping requirement is defined as the percentage of time the aircraft can remain on station, which is the percentile wind speed, $p_{\text{wind}}$. 
...
Because becuase wind speeds are greatest during the winter solstice for both the Northern and Southern Hemispheres only winds speeds from December and June respectively, will be used for the aircraft sizing and performance analysis presented in this paper. 
...
Gas powered aircraft generally fly in the lower altitude regime of 4,600-7,600 m because naturally aspirated engines lose power with increased altitude and are unable to reach altitudes higher than about 13,500 m.  
...
\begin{figure}[H]
	\begin{center}
	\includegraphics[width=0.6\textwidth,natwidth=538,natheight=405]{altvswindhmin.eps}
...
\begin{figure}[H]
	\begin{center}
	\includegraphics[width=0.6\textwidth,natwidth=576,natheight=432]{latvswindN.eps}
    \caption{\textbf{Wind speeds by latitude.  Bands represent 80th, 90th and 95th percentile winds.\cite{wind} }}
	\label{f:latvswind}
	\end{center}
\end{figure}

Latitude additionally affects the solar-electric powered aircraft because at higher latitudes there is less daylight and therefore less solar energy during the winter months. This is further discussed in Section IV.A.
...
% \begin{figure}[H]
%  \begin{subfigmatrix}{2}% number of columns
%      % \subfigure[Solar Weight Breakdown\label{f:solarwbreak}]{\includegraphics[width=1.0\textwidth,natwidth=144,natheight=320]{solarwbreak.pdf}}
%      % \subfigure[Gas Weight Breakdown\label{f:gaswbreak}]{\includegraphics[width=1.0\textwidth,natwidth=178,natheight=320]{gaswbreak.pdf}}
%      \subfigure[Solar Weight Breakdown\label{f:solarwbreak}]{\includegraphics[width=0.30\textwidth]{solarwbreak.pdf}}
%      \subfigure[Gas Weight Breakdown\label{f:gaswbreak}]{\includegraphics[width=0.40\textwidth]{gaswbreak.pdf}}
%  \end{subfigmatrix}
%  \caption{\textbf{Representative weight breakdown of both aircraft optimization models.}}
%  \label{f:wbreak}
% \end{figure}
...
Altitude can be post-calculated from the optimized air density. (See Appendix A) 
...
\begin{figure}[H]
	\begin{center}
	\includegraphics[width=0.7\textwidth,natwidth=572,natheight=405]{lat30.eps}
...
Thus, for a given latitude and day, and therfore total solar energy $(E/S)_{\text{sun}}$, both $(E/S)_{\text{day}}$ and $(E/S)_{\text{twilight}}$ are functions of $(P/S)_{\text{min}}$.  
...
The average weight, $W_{\text{ave}}$, is assumed to be the geometric mean, defined as
...
The non-wing drag coefficient $C_{d_0}$, is estimated as a drag build up from the fuselage, tail boom, and horizontal and vertical surfaces discussed later.
    ...
    To estimate the wing profile drag, a posynomial fit is made of the drag polar of the selected \emph{JH01} airfoil (See Appendix D for a discussion of the airfoil choice). 
...
\item{Standard Wing Bending Case}
...
\item{Cap Spar for Bending Loads}
...
Additional wing weight, $W_{\text{fadd}}$, accounts for additional structural weight in the wing,
...
% \begin{align}
%     \label{e:weightmtow}
%     W_{\text{MTO}} &\geq W_{\text{structural}}  + W_{\text{payload}} + W_{\text{fuel}} + W_{\text{engine}} \\
...
% \begin{align}
%     \label{e:weightsmtow}
%     W_{\text{MTO}} &\geq W_{\text{structural}} + W_{\text{payload}} + W_{\text{solar}} + W_{\text{batt}} + W_{\text{motor}} \\
...
The gas powered model was solved 63 times in 5.6178 seconds and the solar-electric powered model was solved 31 times in 2.5529 seconds to produce Figure~\ref{f:latvsmtowtrade}.

\begin{figure}[H]
	\begin{center}
	\includegraphics[width=0.6\textwidth,natwidth=528,natheight=405]{mtowvslat.eps}
...
\begin{figure}[H]
	\begin{center}
	\includegraphics[width=0.6\textwidth,natwidth=527,natheight=405]{mtowvsendurance.eps}
...
Figure~\ref{f:battsolarcontour} was produced using 157 separate optimiation solutions that took a total of 14.379 seconds to solve. 
...
The infeasible regions and contour shapes would change for different assumed constant values. 
 ...
 \begin{figure}[H]
 \begin{subfigmatrix}{3}% number of columns
  \subfigure[35th Latitude, 80\% Wind Speed]{\includegraphics[natwidth=542,natheight=414]{bcontourl35a80.eps}}
  \subfigure[35th Latitude, 85\% Wind Speed]{\includegraphics[natwidth=542,natheight=414]{bcontourl35a85.eps}}
  \subfigure[35th Latitude, 90\% Wind Speed]{\includegraphics[natwidth=542,natheight=414]{bcontourl35a90.eps}}
  \subfigure[30th Latitude, 80\% Wind Speed]{\includegraphics[natwidth=542,natheight=414]{bcontourl30a80.eps}}
  \subfigure[30th Latitude, 85\% Wind Speed]{\includegraphics[natwidth=542,natheight=414]{bcontourl30a85.eps}}
  \subfigure[30th Latitude, 90\% Wind Speed]{\includegraphics[natwidth=542,natheight=414]{bcontourl30a90.eps}}
  \subfigure[25th Latitude, 80\% Wind Speed]{\includegraphics[natwidth=542,natheight=414]{bcontourl25a80.eps}}
  \subfigure[25th Latitude, 85\% Wind Speed]{\includegraphics[natwidth=542,natheight=414]{bcontourl25a85.eps}}
  \subfigure[25th Latitude, 90\% Wind Speed]{\includegraphics[natwidth=542,natheight=414]{bcontourl25a90.eps}}
 \end{subfigmatrix}
...
\begin{figure}[H]
	\begin{center}
	\includegraphics[width=0.6\textwidth,natwidth=519,natheight=405]{windaltoper2.eps}
...
\begin{figure}[H]
	\begin{center}
	\includegraphics[width=0.6\textwidth,natwidth=555,natheight=415]{polarmission2.eps}
...
Many constraints in this optimization study are convex equations that approximate data or other functions.  
...
All fitted equations are a combination of \emph{max-affine} and \emph{softmax-affine} functions,
...
\subsection{Solar energy calculations}
...
\begin{figure}[H]
 \begin{subfigmatrix}{2}% number of columns
     % \subfigure[$C_L=0.0$\label{f:cpmin}]{\includegraphics[width=0.35\textwidth,natwidth=612,natheight=792]{cpmin.eps}}
     % \subfigure[$C_L=1.35$\label{f:cpmax}]{\includegraphics[width=0.35\textwidth,natwidth=612,natheight=792]{cpmax.eps}}

...
%  \usepackage[dvips]{dropping}% alternative dropped capital package
 ...
 \author{
  ...
  \ and Warren Hoburg\thanks{Assistant Professor, Aeronautical and Astronautical Engineering, 77 Mass Ave, Cambridge MA, 02139, AIAA Member.}\\
  {\normalsize\itshape
...
Engineering firms like Aurora Flight Sciences\cite{orion} and Vanailla Aircraft\cite{vanilla} have developed gas powered long-endurance platforms as a means of providing continual survellience for days at a time.  
...
Additionally, because few long-endurance gas powered aircraft have been built and only Airbus's Zephyr\cite{zephyr} solar aircraft has been successfully flown, the operational limits of these aircraft is relatively unknown. 
...
Because gas-powered aircraft are not affected by the solar flux, their station-keeping ability at different latitudes only depends on wind speed. 
...
For a solar-electric powered aircraft, however, if it can fly during the shortest day of the year, December 21st, then it can theortically fly at any time the year.  
...
Because of a naturally aspirated engine and linearly increasing wind speeds from 0 to 35,000 ft shown in Figure~\ref{f:altvswind} gas powered aircraft will generally be more efficiect at lower altitudes and generally fly at a given latitude to meet a payload requirement.  
For this design study, it is assumed that the aircraft is flying at 15,000 ft. 

Solar-electric powered aircraft tend to fly around 60,000 ft to meet payload requirements, avoid cloud coverage and fly in lower wind speeds which have a minimum around 60,000 shown in Figure~\ref{f:altvswind}.  For the solar-electric powered aircraft altitude is optimized as explained in Section. 
...
A python package for defining and manipulating geometric programs, GPkit\cite{gpkitdocs}, is used to create the models described in this paper.  A commerical solver, mosek\cite{mosek}, is used to solve the GP. \\
...
Thus, an aircraft optimized for $\pm$30 will have 11 different wind speed and solar flux constraints that must be satisfied, one for each degree of latitude. 
...
This verison comes from assuming that $\text{BSFC}$ and the power to weight ratio, $(P_{\text{shaft}}/W)$, are constant during the considered flight segment. 
...
\end{align}
    ...
    Equations~\eqref{e:brzbre} and~\eqref{e:brtaylor} are monomial and posynomial respectively and therefore GP compatible. \\
    
    For long-endurance aircraft, missions can last days, causing the power to weight ratio $(P_{\text{shaft}}/W)$ to vary significantly during the course of the flight.  
...
\begin{align}
    \label{e:weightsmtow}
    \text{MTOW} &\geq W_{\text{structural}} + W_{\text{payload}} + W_{\text{solar}} + W_{\text{battery}} + W_{\text{motor}} \\
...
The first is break specific fuel consumption, $\text{BSFC}$; a lower $\text{BSFC}$ will result in increased endurance.  
...
where $\text{BSFC}_{\text{min}} = 0.3162$ [lb/hp/hr].\cite{rcvengines}
...
\begin{figure}[H]
 \begin{subfigmatrix}{2}% number of columns
     \subfigure[Gas Powered\label{f:spanvslatgas}]{\includegraphics{spanvslatgas.pdf}}
     \subfigure[Solar-Electric Powered\label{f:spanvslatsolar}]{\includegraphics{spanvslatsolar.pdf}}
 \end{subfigmatrix}
...
\begin{figure}[H]
	\begin{center}
	\includegraphics[width=0.6\textwidth]{spanvsendurance.pdf}
...
\begin{figure}[H]
 \begin{subfigmatrix}{2}% number of columns
     \subfigure[Wingspan\label{f:span}]{\includegraphics{span.pdf}}
     \subfigure[Solar Cell Area\label{f:solarcell}]{\includegraphics{solarcell.pdf}}
 \end{subfigmatrix}
...
For example, if the objective function were $MTOW$ and the sensitivity to battery energy density were 0.5, then a 1\% increase in the solar cell efficiecy would result in a 0.5\% decrease in weight.  

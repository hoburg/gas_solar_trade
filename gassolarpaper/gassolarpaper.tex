...
\begin{figure}[H]
    ...
    \caption{\textbf{Wind speeds by month at 30 degrees latitude.  Bands represent 80th, 90th and 95th percentile winds.}}
	\label{f:windvsmonth}
	\end{center}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{\textbf{Wind speeds by altitude. Bands represent 80th, 90th and 95th percentile winds.}}
	\label{f:altvswind}
	\end{center}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{\textbf{Posynomial approximation to wind speed data comparison at 35$^{\circ}$ latitude. (Log space RMS error = 0.043.)}}
	\label{f:windfitl35}
	\end{center}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{\textbf{Posynomial fit (solid lines) to XFOIL data (circles) comparison.  (Log-space RMS error~=~0.00489.)}}
	\label{f:JH01polar}
	\end{center}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{\textbf{Monomial approximation to Equation~\ref{e:gustalpha} comparison for $V/V_{\text{gust}} \in [0,0.7]$. (Log-space RMS error = 0.0246)}}
	\label{f:arctanfit}
	\end{center}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{\textbf{Posynomial equation (solid lines) to XFOIL data (circles) comparison. (Log space RMS error = 0.0173)}}
...
\begin{figure}[H]
    ...
    \caption{\textbf{Power law fit to University of North Dakota data\cite{gasengine} comparison. (Log space RMS error = 0.34)}}
...
\begin{figure}[H]
    ...
    \caption{\textbf{Representative engine performance function based on RCV Engine Ltd data.  (Log-space RMS error = 0.007) }}
	\label{f:powervsweightfit}
	\end{center}
\end{figure}
...
To compare the gas-powered and solar-electric architectures the optimization models were solved by minimizing the max take off weight across different bands of latitudes. Figure~\ref{latvsmtowtrade} shows this trade study evaluated at the 80th, 90th and 95th percentile wind speeds.  
Each point on the graph is a unique solution to the optimization model for the stated assumptions and contants.
...
\begin{figure}[H]
    ...
    \caption{\textbf{Gas optimization model feasible for all latitudes. Next integer latitude value for each solar-electric curve is infeasible.}}
    \label{f:latvsmtowtrade}
	\end{center}
\end{figure}
...
\begin{figure}[H]
 ...
 \caption{\textbf{Comparison of structural fraction and detailed structural models highlights trade between wing weight and air density.}}
 \label{f:altoper}
	\end{center}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{\textbf{Wind speed constraint moves lift to drag ratio from optimum endurance ratio $\text{max}(C_L^{1.5}/C_D)$ (plus sign). Solid lines are drag polars.}}
...
Each point in Figure~\ref{f:solarcontours} is a unique design and was solved by minizing wing span. 
Values of constants other than solar cell efficiency are given throughout the text.
The infeasible regions and contour shapes would charge for different assumed constant values. 
 ...
 \end{subfigmatrix}
 \caption{\textbf{Operating at higher latitudes requires both high performing batteries and solar cells.}}
 \label{f:solarcontours}
\end{figure}

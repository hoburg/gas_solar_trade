...
\begin{figure}[H]
    ...
    \caption{\textbf{Variation of wind speed by month at 45 degrees latitude at altitudes of 15,000 ft, 50,000 ft and 60,000 ft.  Lower, middle and upper lines represent 85th, 90th and 95th percentile winds respectively.}}
	\label{f:windvsmonth}
	\end{center}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{\textbf{Variation of wind speed with altitude at latitudes of 30, 35, and 45 degrees.  Left, middle and right lines represent 85th, 90th and 95th percentile winds respectively. Wind data includes winds during December for the years 2005-2015.}}
	\label{f:altvswind}
	\end{center}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{\textbf{Variation of wind speed with latitude at altitudes of 15,000 ft, 50,000 ft and 60,000 ft.  Left, middle and right lines represent 85th, 90th and 95th percentile winds respectively. Wind data includes winds during December for the years 2005-2015.}}
	\label{f:latvswind}
	\end{center}
\end{figure}
...
For example, an aircraft optimized for $\pm$30 will have 11 different wind speed and solar flux constraints that must be satisfied, one for each degree of latitude between 20 and 30 degrees. 
...
\begin{figure}[H]
 \begin{subfigmatrix}{2}% number of columns
     ...
     \subfigure[Monomial Approximation for $(E/S)_{\text{twilight}}$\label{f:Cenergy}]{\includegraphics{Cenergy.pdf}}
 \end{subfigmatrix}
 \caption{\textbf{Monomial approximations for $(E/S)_{\text{day}}$ and $(E/S)_{\text{twilight}}$ as functions for $(P/S)_{\text{min}}$ at 30 degrees latitude. (Log-space RMS error = 0.007 and 0.0180)}}
...
\begin{figure}[H]
    ...
    \caption{\textbf{The posynomial fit, Equation~\eqref{e:aerodragprof}, of the \emph{JH01} airfoil drag polar is represented as solid lines at different Reynolds numbers.  The open circles are the calculated drag polars from XFOIL. (Log space RMS error~=~0.00489.)}}
	\label{f:JH01polar}
	\end{center}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{\textbf{Comparison of monomial approximation to Equation~\ref{e:gustalpha} for $V/V_{\text{gust}} \in [0,0.7]$. (Log space RMS error = 0.0246)}}
	\label{f:arctanfit}
	\end{center}
\end{figure}
...
A cap spar is considered for the solar-electric and gas powered aircraft.  Both aircraft optimization models share the same governing equations.  A cap spar has two carbon fiber caps separated by a foam core as seen in Figure~\ref{f:capspar}.
...
\begin{figure}[H]
    ...
    \caption{\textbf{Comparison of power law fit and engine data from University of North Dakota technical report.\cite{gasengine} (Log space RMS error = 0.34)}}
...
To compare the gas-powered and solar-electric architectures the optimization models were solved by minimizing the max take off weight across different bands of latitudes, shown in Figure~\ref{f:latvsmtowtrade}. The gas powered optimization model contains 606 free variables and the solar-electric powered optmization model contains between 165 and 585 free variables depending on latitude band requirement. 

\begin{figure}[H]
	\begin{center}
	\includegraphics[width=0.6\textwidth]{mtowvslat.pdf}
    \caption{\textbf{Trade study comparing the lowest possible max take off weight across a range of possible latitude band requirements. Each point is a unique solution from the optimization. Lower, middle and upper curves were evaluated at the 80th, 90th and 95th percentile winds respectively.  The endurance requirement for the gas powered aircraft in this trade study is 7 days. The next integer latitude value for each of the solar-electric curves is infeasible for the given assumptions.}}
    \label{f:latvsmtowtrade}
	\end{center}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{\textbf{Trade study of endurance and size for gas powered architecture., Lower, middle and upper curves were evaluated at the 80th, 90th and 95th percentile winds respectively. Max take off weight was minimized for this analysis.}}
...
\begin{figure}[H]
 \begin{subfigmatrix}{2}% number of columns
     \subfigure[Wingspan\label{f:span}]{\includegraphics{solarobjcomp.pdf}}
     \subfigure[Solar Cell Area\label{f:solarcell}]{\includegraphics{solarobjcomp2.pdf}}
 \end{subfigmatrix}
    \caption{\textbf{Solar aircraft sizing study for two different objective functions: wing span $b$, and solar cell area $S_{\text{solar}}$. Lower, middle and upper curves were evaluated at the 80th, 90th and 95th percentile winds respectively.}}
...
\begin{figure}[H]
	\begin{center}
	\includegraphics[width=0.6\textwidth]{windaltoper.pdf}
...
\begin{figure}[H]
	\begin{center}
	\includegraphics[width=0.6\textwidth]{polarmission.pdf}
    \caption{\textbf{Circles and squares represent the optimized drag and lift coefficient values during the course of mission with and without the wind speed constraint. Solid lines are drag polars at various Reynolds numbers.  The maximum $C_L^{1.5}/C_D$ are marked by a plus sign on the polar curve.}}
...
Figure~\ref{f:solarcontours} shows a matrix contour map of the solar-electric powered aircraft for multiple solar cell efficiencies, battery energy densities, latitudes, and percentile wind speeds.
 ...
 \begin{subfigmatrix}{3}% number of columns
  ...
  \subfigure[25th Latitude, 90\% Wind Speed]{\includegraphics{bcontourl25a90.pdf}}
 \end{subfigmatrix}
 \caption{\textbf{Matrix of contour plots of battery specific energy and solar cell efficiency vs wing span for solar-electric powered aircraft where the wing span is minimized. Each point on the graph corresponds to a unique design. Values of constants other than solar cell efficiency and battery specific energy are given throughout the text.  The infeasible region and contour shapes would change for different assumed constant values.}}
 \label{f:solarcontours}
\end{figure}
